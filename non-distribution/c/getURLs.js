...
// Given the HTML of one webpage,
// find all the links on it, turn them into full URLs,
// and print them so the crawler knows where to go next.

const readline = require('readline');
const {JSDOM} = require('jsdom'); // treat html as a DOM, ie find actual elements like <a>
const {URL} = require('url'); // build in tool for handling URLs

// 1. Read the base URL from the command-line argument using `process.argv`.
let baseURL = process.argv[2] || '';

if (baseURL.endsWith('index.html')) { // remove index.html if present, since itd relativize links to that subpage 
